...
MAX_BIND_RETRY = 10
BIND_SLEEP = 0.25
def _run_single_test(certificate_file, ca_file, script, port, post_fn):
    thread = None
    ...
    for i in range(MAX_BIND_RETRY):
        ...
        thread = server.ServerThread(certificate_file, port, event)
        thread.start()
    ...
    if not thread.bound:
        return None
    try:
        print script, port, ca_file
        output = subprocess.check_output([script,"127.0.0.1","%d" % (port), ca_file], stderr=subprocess.PIPE)
        print script + " "+output
...
def run_test(certificate_file, test_scripts, ca_file, starting_port = 10000, pool_size=4, pool = None):
    results = []
    close = False
    if pool is None:
        pool = multiprocessing.Pool(pool_size)
        close = True
    args = ( (certificate_file, ca_file, script, starting_port + i) for i, script in enumerate(test_scripts))
...
def test_dir(certificate_dir, test_scripts, ca_dir, starting_port = 10000, ending_port = 20000, pool_size = 4):
    def build_args(certs, scripts):
        i = -1
        for cert in certs:
            for j,script in enumerate(scripts):
                i += 1
                yield (cert, j), (os.path.join(certificate_dir, cert)\
...
if __name__ == "__main__":
    ...
    res = run_test(cert, test_scripts, cafile, pool_size=1, starting_port=30000)
    print(res)
